...
<style>
  /* Basic styling for the product listing */
  .product-listing-m {
...
window.onload = function() {
    var value = localStorage.getItem('buttonValue');
...
<?php 
    ...
    if (isset($_GET['carType'])) {
        $carType = htmlspecialchars($_GET['carType']); // Sanitize the input to prevent XSS
...
//         WHERE tblvehicles.VehiclesBrand = $carType";

if ($carType >= 1 && $carType <= 100) {
  $sql = "SELECT tblvehicles.*, tblbrands.BrandName, tblbrands.id as bid 
          FROM tblvehicles 
          JOIN tblbrands ON tblbrands.id = tblvehicles.VehiclesBrand 
          WHERE tblvehicles.VehiclesBrand = $carType";
